...
fn extract_library() -> String {
    ...
    let fp = temp_dir().as_path().join(file_name);
    let mut file = File::create(fp.clone()).unwrap();
    let bytes = include_bytes!(env!("CARGO_CDYLIB_FILE_MIRRORD_LAYER"));
    file.write_all(bytes).unwrap();
    debug!("Extracted library file to {:?}", &fp);
...
fn main() {
    ...
    std::env::set_var("MIRRORD_IMPERSONATED_POD_NAME", args.pod_name);
    let library_path = extract_library();
    add_to_preload(&library_path).unwrap();
    args.binary_args.insert(0, args.binary.clone());
    let err = execvp(args.binary, args.binary_args);
